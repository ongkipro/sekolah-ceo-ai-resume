    ...
    <div className="w-full bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6 shadow-sm space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
        <div>
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mt-1">
            Matriks Dekonstruksi Pekerjaan: Alokasi Tugas AI vs Manusia
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Simulasikan bagaimana dekonstruksi job description memangkas jam kerja repetitif dan menghemat biaya hingga {stats.efficiencyPercent}%.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/50">
          <div className="text-xs font-semibold text-blue-600 dark:text-blue-400">Jam Repetitif Terpangkas</div>
          <div className="text-2xl font-black text-slate-900 dark:text-white mt-1">
            {stats.aiHours} Jam / Minggu
          </div>
        ...
        <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-900/50">
          <div className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">Tingkat Efisiensi Output</div>
        ...
        <div className="p-4 rounded-xl bg-purple-50 dark:bg-purple-950/40 border border-purple-100 dark:border-purple-900/50">
          <div className="text-xs font-semibold text-purple-600 dark:text-purple-400">Estimasi Efisiensi Biaya/Bulan</div>
          <div className="text-2xl font-black text-slate-900 dark:text-white mt-1">
            Rp {(stats.monthlyCostSaved / 1000000).toFixed(1)} Juta
          </div>
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
            Dari gaji dasar Rp {(currentRole.baseSalary / 1000000).toFixed(0)} Juta/bln
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="text-xs font-bold uppercase tracking-wider text-slate-400">
          Daftar Micro-Tasks Posisi: <span className="text-slate-900 dark:text-white">{currentRole.roleName}</span>
        </div>
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
          {currentRole.tasks.map((task, idx) => (
            <div
              key={idx}
              className="p-3.5 bg-white dark:bg-slate-900 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
            >
              <div className="space-y-1 flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-bold text-slate-900 dark:text-white">{task.name}</span>
                  <span className="text-[11px] font-mono text-slate-400">({task.hoursPerWeek} jam/mgg)</span>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400">{task.description}</p>
                <div className="text-[11px] text-slate-400 font-mono">
                  Tool: <span className="text-blue-600 dark:text-blue-400 font-semibold">{task.toolRecommended}</span>
                </div>
              </div>
